...
const AMBIENCE = [
  { t: 1390, text: "你套上制服背心的時候，發現背心是溫的。像剛被誰脫下來。" },
  { t: 1400, text: "店內廣播響起，女聲，語速很慢：「現在時間，二十三點，整。」你記得這家店沒有整點報時。" },
  { t: 1410, text: "飲料櫃的玻璃門上有一個手印，從裡面按的。你打開櫃門，擦掉了。關上門，手印還在。" },
  { t: 1440, text: "店裡的音樂斷了一下。不是切歌——是同一首歌，倒退了一秒，接著放。" },
  { t: 1460, text: "泡麵區有一桶的封膜是鼓的，像裡面裝的是熱的。你捏了一下。是涼的。" },
  { t: 1470, text: "飲料櫃的壓縮機停了。整家店安靜得像被誰按了靜音。三秒後，它自己咳了一聲，繼續運轉。" },
  { t: 1480, text: "打卡機印紙的聲音響了一下。你走過去看，沒有新卡。最後一張是你的，23:00，印得很正。" },
  { t: 1500, text: "雜誌架全是過期的。你翻了翻，最新的一本也是三個月前的。架上沒有灰。" },
  { t: 1530, text: "關東煮的湯滾了一下。你抬頭——還沒到關火的時間，湯面卻是靜的。" },
  { t: 1550, text: "收銀機底下掉出一張便簽。你撿起來，是空白的。你把它貼回去——原來那張便簽還在，貼得好好的。" },
  { t: 1580, text: "自動門的感應聲響了一下。你抬頭，門是關的。感應器上的紅點亮著，像有誰正站在門口。" },
  { t: 1600, text: "打卡機的數字跳了一格，又跳回來。你決定以它為準。不管它剛才想說什麼。" },
  { t: 1610, text: "店內廣播：「現在時間，二十六點，整。」你低頭看打卡機：02:50。" },
  { t: 1640, text: "收銀機抽屜裡多了一枚硬幣，不是你收進來的。幣面上的年份，你沒有見過。" },
  { t: 1680, text: "微波爐嗶了一聲。裡面是空的，轉盤在轉。" },
  { t: 1690, text: "微波爐的顯示幕亮著：04:44。你沒按過。你按了取消。它還是顯示 04:44。" },
  { t: 1700, text: "霜淇淋機的燈滅了。你沒關過它。" },
  { t: 1705, text: "發票機自己開始印，吐了十幾公分的紙，又自己停住。你撕下來。" },
  { t: 1705, kind: "receipt", text: "\n--------------------------------\n\n     還 沒 輪 到 你\n\n--------------------------------\n" },
  { t: 1710, text: "霜淇淋機的燈又亮了。你還是沒有關過它。" },
  { t: 1750, text: "玻璃門外的天黑得很均勻。你忽然發現門上看不見你的倒影——只有店裡的燈。" },
  { t: 1760, text: "你聽見補貨的聲音——塑膠筐、上架、排面。聲音來自你最熟的那排貨架。你今晚還沒補過那排。" },
  { t: 1770, text: "庫存本自己攤開著。你確定你上次把它合上了。攤開的那頁是飲料。" },
  { t: 1790, text: "自動門開了。有風進來，是暖的。像什麼東西剛走過去。" },
  { t: 1830, text: "窗外的天黑得很標準。你忽然不記得，店門口那盞路燈，原本是不是這個顏色。" },
  { t: 1855, text: "打卡機上，你的卡斜插在卡槽裡。你記得你插得很正。" },
];

// 站櫃台詞輪播：同一句刷十幾次會麻,換著講。
const COUNTER_LINES = [
  "你站回收銀台。自動門關著，關東煮的湯在滾，店裡的亮很標準。",
  "你站回收銀台。監視器一台一台輪。六台。沒有第七台。",
  "你站回收銀台。打卡機的數字一格一格走，走得很守規矩。",
  "你站回收銀台。你數了貨架。六排。你又數了一次。六排。",
];
...
function actions(state, ctx) {
  ...
  // 一次性小發現:讓「等下一個事件」的空窗也在累積不對勁。
  if (!state.checkedWarehouse) {
    list.push({ id: "warehouse", label: "去後場點倉庫",
      onChoose: (s, c) => {
        s.checkedWarehouse = true;
        c.narrate("倉庫的門只能推開一半，貨堆得太滿了。你側身擠進去。最裡面的架上疊著一疊制服背心，一件一件，疊得很平整。你數了。七件。這家店連你，三個員工。");
        s.time += 8;
      } });
  }
  if (!state.seenRoster) {
    list.push({ id: "roster", label: "看排班表",
      onChoose: (s, c) => {
        s.seenRoster = true;
        c.narrate("排班表釘在辦公室門後。你的名字寫在今晚的大夜，字跡是你自己的。你往前翻——同一個時段，同一個名字，一天一天，寫了三十幾頁。你不記得你上過那麼多個大夜。再往前的那疊被撕掉了，撕得很整齊。");
        s.time += 5;
      } });
  }
  if (!state.wipedFrost) {
    list.push({ id: "frost", label: "擦冷凍櫃的霜",
      onChoose: (s, c) => {
        s.wipedFrost = true;
        c.narrate("霜結得很厚，厚得像從來沒人開過這個櫃。你擦開一塊——裡面整齊地結著霜花，一格一格，像貨架。你把霜抹回去了。");
        s.time += 6;
      } });
  }
  if (!state.sortedReturns) {
    list.push({ id: "returns", label: "整理退貨筐",
      onChoose: (s, c) => {
        s.sortedReturns = true;
        c.narrate("筐裡有一包沒拆的吐司，保存期限印著昨天。你把它翻過來，期限是明天。你再翻回去。昨天。你把它放到一邊，決定明天再決定。");
        s.time += 4;
      } });
  }
  ...
  list.push({ id: "counter", label: "站回收銀台（時間流逝）",
    onChoose: (s, c) => {
      const n = s._counterN = (s._counterN || 0) + 1;
      c.narrate(COUNTER_LINES[(n - 1) % COUNTER_LINES.length]);
      s.time += 10;
    } });
...
const ENDINGS = [
  ...
  { id: "filled", label: "補齊",
    ...
    text: "你又巡了一次店。每一排都是滿的，每一格都沒有空位——包括那條不存在的走道盡頭，那扇門後面：貨架整齊地排著，最裡面那格，放著一件制服背心，疊得很平整，尺碼是你的。架上不能留空位。這家店，補齊了。" },
